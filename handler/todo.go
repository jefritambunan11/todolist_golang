...
func (h *todoHandler) GetTodo(c *gin.Context) {	
	...
	// todoDetail, err := h.todoService.GetTodoByID(input, userID)
	// if err != nil {
	// 	response := helper.APIResponse("Transaksi Database Gagal", http.StatusBadRequest, "error", nil)
	// 	c.JSON(http.StatusBadRequest, response)
	// }
	var user = c.MustGet("who_is_logged_in").(user.User)	
	var todoDetail, err2 = h.todoService.GetTodoByID(input, user.ID)
	if err2 != nil {
